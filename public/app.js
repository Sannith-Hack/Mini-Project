...
window.addEventListener('DOMContentLoaded', () => {
    // 1. Check if token & username were passed in URL (from Google OAuth callback)
    ...
    // 2. Load token and username from localStorage
...
function initDashboard() {
    ...
    if (welcomeMsg) welcomeMsg.textContent = `Welcome, ${currentUser}`;
...
async function loadGlobalStats() {
    try {
        ...
        const ctx = adminChartEl.getContext('2d');
...
if (downloadPdfBtn) {
    downloadPdfBtn.addEventListener('click', () => {
        try {
            ...
            const level = document.getElementById('result-box').dataset.rawLevel || 'UNKNOWN';
